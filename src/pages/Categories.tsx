  ...
  const priceLabels = priceRanges.map((range) => ({
    ...
    label:
      range.label === 'All Prices'
        ? range.label
        : range.label.replace(/\d+/g, (match) => formatPrice(parseInt(match, 10))),
  }));
      ...
      <div className="container mx-auto px-4 py-8 lg:py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Browse our handcrafted crochet treasures, or explore by category.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
          {categories.map((category, index) => (
            <button
              ...
              className={cn(
                'group relative overflow-hidden rounded-3xl bg-card shadow-soft transition-all duration-300 hover:shadow-hover hover:-translate-y-1',
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-1 border border-border rounded-lg p-1">
              <button
                ...
                className={cn('p-2 rounded-md', gridCols === 3 ? 'bg-muted' : 'hover:bg-muted/50')}
              >
                <LayoutGrid className="h-4 w-4" />
              </button>
              <button
                onClick={() => setGridCols(4)}
                className={cn('p-2 rounded-md', gridCols === 4 ? 'bg-muted' : 'hover:bg-muted/50')}
              >
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={cn(
                    ...
                    selectedSpecial === 'new' ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-muted'
                  )}
                >
                  ✨ New Arrivals
                </button>
                <button
                  ...
                  className={cn(
                    'w-full text-left px-4 py-2 rounded-lg text-sm',
                    selectedSpecial === 'bestsellers' ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-muted'
                  )}
                >
                  🔥 Best Sellers
                </button>
              ...
              <div className="space-y-2">
                {priceLabels.map((range, index) => (
                  <button
                    ...
                    className={cn(
                      'w-full text-left px-4 py-2 rounded-lg text-sm',
                      selectedPriceIndex === index ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-muted'
                    )}
                  >
                    {range.label}
                  </button>
