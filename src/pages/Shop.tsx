    ...
    <Layout>
      <div className="container mx-auto px-4 py-8 lg:py-12">
        <div className="text-center mb-8 lg:mb-12">
          <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Shop Our Collection</h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Browse our handcrafted crochet treasures, each piece made with love and attention to detail.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-border">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-1 border border-border rounded-lg p-1">
              <button onClick={() => setGridCols(3)} className={cn("p-2 rounded-md", gridCols === 3 ? "bg-muted" : "hover:bg-muted/50")}><LayoutGrid className="h-4 w-4" /></button>
              <button onClick={() => setGridCols(4)} className={cn("p-2 rounded-md", gridCols === 4 ? "bg-muted" : "hover:bg-muted/50")}><Grid3X3 className="h-4 w-4" /></button>
            </div>
        ...
        <div className="flex gap-8">
          <aside className={cn("w-64 space-y-6", showFilters ? "block" : "hidden lg:block")}> 
            <div>
              ...
              <div className="space-y-2">
                <button onClick={() => updateFilter("category", "all")} className={cn("w-full text-left px-4 py-2 rounded-lg text-sm", selectedCategory === "all" ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>All Products</button>
                {categoryList.map((cat) => (
                  <button key={cat.id} onClick={() => updateFilter("category", cat.id)} className={cn("w-full text-left px-4 py-2 rounded-lg text-sm flex items-center gap-2", selectedCategory === cat.id ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>
            ...
            <div>
              ...
              <div className="space-y-2">
                <button onClick={() => updateFilter("special", "new")} className={cn("w-full text-left px-4 py-2 rounded-lg text-sm", selectedSpecial === "new" ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>✨ New Arrivals</button>
                <button onClick={() => updateFilter("special", "bestsellers")} className={cn("w-full text-left px-4 py-2 rounded-lg text-sm", selectedSpecial === "bestsellers" ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>🔥 Best Sellers</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {priceLabels.map((range, index) => (
                  <button key={range.label} onClick={() => updateFilter("price", index.toString())} className={cn("w-full text-left px-4 py-2 rounded-lg text-sm", priceIndex === index ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>{range.label}</button>
                ))}
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1">
            {filteredProducts.length > 0 ? (
              <div className={cn("grid gap-6", gridCols === 3 ? "lg:grid-cols-3" : "lg:grid-cols-4", "grid-cols-1 sm:grid-cols-2")}>
